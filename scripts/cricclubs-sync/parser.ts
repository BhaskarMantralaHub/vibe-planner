...
// One upcoming fixture from cricclubs' fixtures.do page.
// Used to refresh date/time/venue/umpire for unplayed matches that the team
// admin may have moved on cricclubs after the local schedule row was created.
export type ParsedFixture = {
  cricclubs_fixture_id: number;     // stable cricclubs row id (deleteRow{N})
  match_type: string | null;        // 'League' | 'Practice' | ...
  match_date: string | null;        // ISO YYYY-MM-DD
  match_time_24h: string | null;    // 'HH:MM' (24h) — schedule schema format
  team_home: string;                // cricclubs Team 1 (Home) column
  team_away: string;                // cricclubs Team 2 column
  venue: string | null;
  umpire1: string | null;
  umpire2: string | null;
};
...
// ── Fixtures (upcoming) parser ──────────────────────────────────────────
//
// cricclubs' fixtures.do page renders the same data twice: a card layout
// and a clean table at `#schedule-table1` (upcoming) / `#schedule-table`
// (past). We only consume the upcoming table — past matches flow through
// the listMatches.do path which carries scores.

const parseTime12To24 = (raw: string): string | null => {
  // "2:45 PM" → "14:45", "10:45 AM" → "10:45"
...
export const parseFixtures = (html: string): ParsedFixture[] => {
  ...
  // The upcoming-matches table — past matches live in `#schedule-table`
