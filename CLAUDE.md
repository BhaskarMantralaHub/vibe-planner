...
- **PARTIALLY wired.** Writers are done: `addPlayer` enrols into the selected season, and `PlayerManager`'s ⋮ menu has "Add to / Remove from &lt;Season&gt;" (`enrollInSeason` / `removeFromSeason`, both returning a boolean so a failed write cannot toast success). Readers done: **dues** (`FeeTracker`), the dashboard **Players** count, and `PlayerManager`'s **Squad list** (season-scoped via `seasonRoster`; active members without a roster row move to a collapsible "Not in &lt;Season&gt;" section with a one-tap "Add back", which disappears entirely in fallback mode), via `seasonRoster` / `billableRoster` in `app/(tools)/cricket/lib/season-roster.ts`. Also roster-scoped now: `.github/scripts/send-monthly-report.sh` (recipients AND fee stats filter to the active season's billable roster via one `$PLAYERS` filter, with the same empty-roster fallback — an off-roster club member gets neither the email nor a fee line). Still team-wide: umpiring (`computeDutyStats`), splits and gallery pickers, `PlayerManager`'s Guests tab, and scoring XI.
...
- **Captain / vice-captain are SEASON facts now** (`cricket_season_players.designation`, migration in `docs/season-designation-migration.sql` — council-reviewed). Same two-facts playbook as `is_guest`: `cricket_players.designation` is KEPT as the **CURRENT** designation (fallback mode, cross-team RPC payloads) and is mirrored from an ACTIVE season by the atomic `set_season_designation` RPC (SECURITY INVOKER, TEXT reason codes, displaces the incumbent in the same transaction; partial unique indexes `uniq_season_captain`/`uniq_season_vice_captain` scoped `left_at IS NULL` make one-per-season structural). Editing a historical season touches only that season's row. ALL readers go through `seasonRoster(...).designationOf(playerId)` — PlayerManager chips + form, the dashboard greeting, ShareButton's `(C)`/`(VC)` tags (text + PDF), PlayerProfile, DutyPlayerSheet — never `player.designation` directly (that one is only the fallback the helper itself reads). A DB trigger clears live season designations on deactivate/move-to-guest so a departed captain can't occupy the unique slot forever; a mid-season departure via "Remove from Season" (`left_at`) keeps `designation` on the departed row as history. The form's designation write goes through `setSeasonDesignation` AFTER `updatePlayer`/`addPlayer` — never in those payloads (a stray `designation: null` in the update payload would wipe the mirror on every edit).
...
- **Cricket global nav — ONE vocabulary, every page.** `CRICKET_GLOBAL_NAV` in `app/(tools)/cricket/components/cricket-global-nav.ts` (Roster / Finances / Matches / Umpiring / Moments — the first item's KEY stays `'players'`; only the label is "Roster", and it uses the lucide `Users` icon to match the hamburger's Roster entry, Wallet for Finances) is what every cricket page passes to `CricketSectionNav`; only the dashboard keeps view-kind items for Roster/Finances (same five destinations). "Roster" = navigation destination, "Squad" = the content section inside it; "Cricket" is no longer a label anywhere (pinned by `tests/unit/lib-nav.test.ts`). The hamburger drawer groups tools via the presentation-only `group` field in `lib/nav.tsx` (Team / Team management / Game day / Personal / Administration). Section-internal views (Upcoming/Completed/Stats on Matches; league-stats is the Matches section's `stats` view, dock stays on `matches`) are CONTEXTUAL navigation and render as a `SegmentedControl` at the top of the content — never as a second bottom bar and never by swapping the dock's items. The dock's active state is a measured sliding tint pill; the SegmentedControl's is a translateX-driven elevated chip.
- **`--cricket-on` = text/icon color on a `--cricket` fill** (white in light, dark ink in dark — the dark theme's luminous orange fails contrast under white text). Any new filled brand control (button, chip, FAB) must use `text-[var(--cricket-on)]`, never `text-white`.
