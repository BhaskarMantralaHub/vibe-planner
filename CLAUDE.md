...
| iOS Shortcut + Edge Function (deprecated, replaced by Scriptable) | `supabase/functions/cricclubs-ingest/` (Deno + cheerio) was the V1 fixtures-only path: iOS Shortcut fetched `fixtures.do`, POSTed HTML to the function with `X-Sync-Secret` header, function ran `refreshFixtures()` and returned a notification summary. Superseded by the Scriptable script which does fixtures + scorecards in one tap; Shortcut on the home screen can be deleted. Edge Function file stays deployed as a dormant backup endpoint — the Scriptable script doesn't call it. |
