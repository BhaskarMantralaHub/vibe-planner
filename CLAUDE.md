...
| iOS Shortcut sync | `supabase/functions/cricclubs-ingest/` is a Supabase Edge Function (Deno + cheerio) that accepts cricclubs HTML POSTed by an iOS Shortcut on the team admin's iPhone. The phone fetches cricclubs.com directly (residential IP — bypasses the Cloudflare challenge that breaks GitHub Actions) and POSTs raw HTML to the function with an `X-Sync-Secret` header. The function parses, runs the same `refreshFixtures()` logic as `sync.ts` for parity, and returns a pre-formatted two-line `summary` string the Shortcut shows as a notification. Auth: shared secret stored as `CRICCLUBS_SYNC_SECRET` via `supabase secrets set`; `verify_jwt = false` in `config.toml` for this function only. V1 is fixtures-only (upcoming schedule refresh). V2 will add list + scorecard endpoints to also auto-complete past matches from the iPhone. |
