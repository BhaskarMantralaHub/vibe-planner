...
- **Cricket global nav — ONE vocabulary, every page.** `CRICKET_GLOBAL_NAV` in `app/(tools)/cricket/components/cricket-global-nav.ts` (Players / Finances / Matches / Umpiring / Moments) is what every cricket page passes to `CricketSectionNav`; only the dashboard keeps view-kind items for Players/Finances (same five destinations). Section-internal views (Upcoming/Completed/Stats on Matches; league-stats is the Matches section's `stats` view, dock stays on `matches`) are CONTEXTUAL navigation and render as a `SegmentedControl` at the top of the content — never as a second bottom bar and never by swapping the dock's items. The dock's active state is a measured sliding tint pill; the SegmentedControl's is a translateX-driven elevated chip.
