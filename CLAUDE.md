...
- **NOT YET WIRED INTO THE APP.** The schema is live and backfilled (Spring 24 = 18 + 6 guests, Fall 18) but **no code reads it** — every roster read is still team-wide via `cricket_players`. Until that switches, adding a player to any season adds them to all of them.
...
- **`cricket_seasons.opening_balance`** — pool money carried in from the previous season. A STATIC admin-set snapshot, NEVER derived live from prior seasons (a live chain would let an edit to an old expense silently rewrite every later balance). **Nullable on purpose**: `restore.yml` rebuilds rows with `json_populate_recordset`, which does not apply defaults, so `NOT NULL` broke restore from every pre-migration backup. Read it as `opening_balance ?? 0`.
- **Pool balance is computed in FOUR places that already disagree** — `page.tsx:216`, `ShareButton.tsx:19-21` (**omits sponsorships — pre-existing money bug**), `ShareButton.tsx:83-85`, `.github/scripts/send-monthly-report.sh:185`. Extract ONE `computePoolBalance({openingBalance, fees, sponsors, expenses})` into `app/(tools)/cricket/lib/utils.ts` with tests BEFORE adding `opening_balance` anywhere, or it becomes four chances to get money wrong.
- **League stats stay CAREER-WIDE by decision** — `cricclubs_batting_season` / `cricclubs_bowling_season` group by `(team_id, player_id)` with no season or league dimension, so the `_season` name is misleading; they are career totals and Fall data will merge into Spring's with no way to separate it. Match-play in `cricclubs_batting`/`_bowling` is deliberately NOT treated as season-roster membership.
...
**Umpiring duties:** `/cricket/umpiring` (hamburger + cricket bottom nav). MTCA assigns each league match TWO umpire slots, naming a *team* per slot; when a slot names us one of our players must stand — at a match we are **not playing**. Stored one row per SLOT in `cricket_umpiring_duties` (`role_slot` 1–4; MTCA only ever produces 1–2, extras are for hand-added duties), because allocation varies: one slot, both slots on one match, or one slot each on several matches at the same time. Tables/RLS/RPCs in `docs/umpiring-schema.sql`; per-season target + our numeric cricclubs team id live in `cricket_umpiring_settings`.
...
| ⚠️ Umpiring NOT yet in the Scriptable sync | `scripts/cricclubs-sync/ingest-html.mts` syncs duties (verified against `__tests__/fixtures/fixtures-league.html`), but `scripts/scriptable/cricclubs-sync.js` does **not** — running it today pulls ZERO duties. Still to port: (1) a second, league-wide `fixtures.do` fetch with no `teamId`, kept separate from the team-filtered one used by `refreshFixtures`; (2) the `teamId` href capture into its inline `FIXTURES_PARSER`; (3) `extractUmpiringDuties` + insert-if-absent / patch-MTCA-facts-only writes; (4) resolve the season and league from `cricket_seasons` (asserting exactly ONE active season for the team) instead of the hardcoded `CONFIG.league_id` — otherwise next season keeps syncing league 87 forever. A new season also needs a `cricket_umpiring_settings` row carrying `cricclubs_team_id`, or the duty sync skips with a warning. |
