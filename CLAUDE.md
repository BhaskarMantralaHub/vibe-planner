...
| Edge Function full-sync (canonical path, 2026-05-18+) | `supabase/functions/cricclubs-ingest/` (Deno + cheerio) is now the single canonical sync implementation. Two routes: `POST ?type=full-sync` (server-driven — fetches cricclubs via Apify residential proxy, parses, upserts everything, auto-completes schedule) and `POST ?type=fixtures` (legacy V1 — accepts pre-fetched HTML body, runs `refreshFixtures()` only). Both routes share a singleton lock via `cricclubs_sync_state` + the `acquire_cricclubs_sync_lock` / `release_cricclubs_sync_lock` SECURITY DEFINER RPCs (migration 009) — token-checked release prevents stale-zombie clobber. Auth is dual: `X-Sync-Secret` header (for iOS Shortcut, pg_cron, GH Action) OR Supabase JWT with `'admin' in profiles.access` (for the in-app "Sync Now" admin button on `/cricket/schedule`). CORS allowlist: `viberstoolkit.com` + localhost. Error strings scrubbed for `?token=`, `Bearer`, `api_key` before persisting to `last_summary` (RLS-readable). Apify routing is critical because Edge Function runs on Deno Deploy datacenter IPs which Cloudflare challenges; Apify exits from residential proxies. Parsers under `parser.ts` are ports of `scripts/cricclubs-sync/parser.ts` with their own Deno parity tests (`__tests__/parser.test.ts`) — when cricclubs HTML drifts, fix both. The legacy Scriptable, GH Action, and Shortcut paths still work as alternative triggers but the Edge Function is the source of truth. |
