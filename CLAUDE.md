...
- **PARTIALLY wired.** Writers are done: `addPlayer` enrols into the selected season, and `PlayerManager`'s ⋮ menu has "Add to / Remove from &lt;Season&gt;" (`enrollInSeason` / `removeFromSeason`, both returning a boolean so a failed write cannot toast success). Readers done: **dues** (`FeeTracker`), the dashboard **Players** count, and `PlayerManager`'s **Squad list** (season-scoped via `seasonRoster`; active members without a roster row move to a collapsible "Not in &lt;Season&gt;" section with a one-tap "Add back", which disappears entirely in fallback mode), via `seasonRoster` / `billableRoster` in `app/(tools)/cricket/lib/season-roster.ts`. Still team-wide: umpiring (`computeDutyStats`), splits and gallery pickers, `PlayerManager`'s Guests tab, scoring XI, and `.github/scripts/send-monthly-report.sh` (widest blast radius — it emails everyone).
